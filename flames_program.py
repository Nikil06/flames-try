...
class HistoryFrame(BaseFrameTab):
    ...
    def addItem(self, _item: HistoryItem):
        '''row_frame = ctk.CTkFrame(self.history_frame)
        label = ctk.CTkLabel(self.history_frame, text = _item.display_str,
                             compound = 'left', anchor = 'w')

        button = ctk.CTkButton(self.history_frame, text = 'Delete', width = 100, height = 24,
                               command = lambda: self.deleteHistoryItem(_item))

        label.pack(row_frame, padx = 0, side = ctk.LEFT)
        label.pack(row_frame, padx = 0, side = ctk.RIGHT)

        self.label_list.append(label)
        self.button_list.append(button)
        self.row_list.append(row_frame)'''
        row_item = HistoryRowItem(_master = self.history_frame, _identity = self.row_control + 1)
        self.row_control += 1

        row_item.label.configure(True, text = _item.display_str)

        self.row_list.append(row_item)

    def removeItem(self, _item: HistoryItem):
        for row_item in self.row_list:
            if _item.identity == row_item.identity:
                row_item.destroy()
                self.row_list.remove(row_item)
                return


class HistoryRowItem(ctk.CTkFrame):
    def __init__(self, _master, _identity):
        super(HistoryRowItem, self).__init__(_master)

        self.identity = _identity
        self.label = ctk.CTkLabel(self, compound = 'left', anchor = 'w')
        self.button = ctk.CTkButton(self, text = 'Delete', width = 100, height = 24,
                                    command = lambda: deleteHistoryItem(self))

        self.label.pack(padx = 0, side = ctk.LEFT)
        self.button.pack(padx = 0, side = ctk.RIGHT)
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        item = HistoryItem('Nikil', 'Aradhana', 'Marriage')
        self.history_frame.addItem(item)
...
def deleteHistoryItem(_item: HistoryRowItem):
    pass
